...
# Search settings (safe, modest defaults)
...
class Flow(StatesGroup):
    CHOOSING_MODE = State()         # start screen
    CHOOSING_CAT  = State()         # browsing categories
    FILTER_PRICE  = State()         # set price range or skip
    FILTER_KIND   = State()         # choose percent vs absolute rubles
    FILTER_VALUE  = State()         # enter the number
...
# In-memory session store (can be replaced by Redis if needed)
...
def already_sent(nm: str) -> bool:
    if not RDS:
        return False
    try:
        return RDS.exists(sent_key(nm)) == 1
    except Exception:
        return False


def mark_sent(nm: str, days: int = 7) -> None:
    if not RDS:
        return
    try:
        RDS.setex(sent_key(nm), days * 86400, "1")
    except Exception:
        pass
...
def scan_catalog_page(page, url: str) -> Tuple[List[str], Dict[str, int], Dict[str, int]]:
    ...
    badge_bonus_rub: nm -> bonus rub (from tile badge, if present)
    approx_price_from_tile: nm -> price rub (from tile text, rough)
    """
    ok = page_open_with_wait(page, url)
    if not ok:
        return [], {}, {}

    # gentle scroll to load tiles
...
async def on_cat_pick(cb: CallbackQuery):
    ...
    kb.adjust(1)
    await cb.message.edit_text(
        "Категория выбрана. Хотите сузить по цене?",
        reply_markup=kb.as_markup(),
    )
...
async def on_skip_to_filters(cb: CallbackQuery):
    ...
    kb.adjust(1)
    await cb.message.edit_text(
        "Хорошо, пропускаем выбор категорий. Зададим цену?",
        reply_markup=kb.as_markup(),
    )
...
def fmt_item(i: dict) -> str:
    ...
    ratio = 0
    if price:
        ratio = int(100 * (bonus or 0) / max(price, 1))
    return (
